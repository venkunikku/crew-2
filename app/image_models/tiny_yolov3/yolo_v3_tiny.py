...
def parseTinyYoloV3Output(output_node_results, filtered_objects, source_image_width, source_image_height, scaled_w,
                          scaled_h, detection_threshold, num_labels):
    ...
    for result_counter in range(output_size):
        ...
        for anchor_boxes in range(num_anchor_boxes_per_cell):
            ...
            x_center = (col + output_node_results[0][row][col][
                anchor_boxes * num_of_classes + 5 + 0]) / output_w * scaled_w
            y_center = (row + output_node_results[0][row][col][
                anchor_boxes * num_of_classes + 5 + 1]) / output_h * scaled_h
            width = np.exp(output_node_results[0][row][col][anchor_boxes * num_of_classes + 5 + 2]) * anchors[
                anchor_offset + 2 * anchor_boxes]
            height = np.exp(output_node_results[0][row][col][anchor_boxes * num_of_classes + 5 + 3]) * anchors[
                anchor_offset + 2 * anchor_boxes + 1]
...
def main_run_model(q, conf):
    ...
    while True:
        ...
        scaled_w = int(
            source_image_width * min(network_input_w / source_image_width, network_input_w / source_image_height))
        scaled_h = int(
            source_image_height * min(network_input_h / source_image_width, network_input_h / source_image_height))
        ...
        for output_node_results in all_output_results.values():
            parseTinyYoloV3Output(output_node_results, filtered_objects, source_image_width, source_image_height,
                                  scaled_w, scaled_h, detection_threshold, len(label_list))
        ...
        for object_index in range(len(filtered_objects)):
            if filtered_mask[object_index] == True:
                ...
                # cv2.putText(display_image, label_list[class_id] + ': %.2f' % confidence, (xmin + 5, ymin + 15),
